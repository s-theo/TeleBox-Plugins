...
const parseImageRequirementAnalysis = (text: string): string[] => {
  ...
  const parseValue = (value: any): string[] => {
    ...
    const prompts = source.map((item: any) => (typeof item === 'string' ? item.replace(/\s+/g, ' ').trim() : '')).filter(Boolean)
...
const extractErrorMessage = (error: any): string => {
  ...
  const reasonText = typeof error?.cause === 'string' ? error.cause : error?.cause ? String(error.cause) : error?.config?.signal?.reason ? String(error.config.signal.reason) : ''
...
const formatErrorForDisplay = (error: any): string => {
  if (error instanceof UserError || error?.name === 'AbortError' || (typeof error?.message === 'string' && error.message.toLowerCase().includes('aborted'))) {
...
const collectImagePartsFromSingleMessage = async (msg: Api.Message, out: AIContentPart[]): Promise<void> => {
  ...
  if (msg.media instanceof Api.MessageMediaDocument && msg.media.document instanceof Api.Document) {
    ...
    const fileName = doc.attributes?.find((attr) => attr instanceof Api.DocumentAttributeFilename)?.fileName || ''
    const isImageFile = docMime.startsWith('image/') || /\.(?:jpe?g|png|webp|gif|bmp|tiff?|avif|heic|heif)$/i.test(fileName)
    const isAnimated = docMime === 'image/gif' || docMime === 'video/webm' || docMime === 'application/x-tgsticker' || docMime === 'application/x-tg-sticker' || doc.attributes?.some((attr) => attr instanceof Api.DocumentAttributeAnimated)
  ...
  for (const candidate of candidates) {
    const photos = await Promise.all(
      [true, false].map(async (isBig) => {
        try {
          return await withTimeout(client.downloadProfilePhoto(candidate, { isBig }), 12000)
        } catch {
          return null
        }
      })
    )
    const photoBuf = photos.find((photo): photo is Buffer => Buffer.isBuffer(photo) && photo.length > 0)
    if (photoBuf) return photoBuf
  }
  return null
}

const getAvatarImagePartFromMessageSender = async (msg?: Api.Message): Promise<AIContentPart | null> => {
...
const replaceStatusMessage = async (current: Api.Message | undefined, msg: Api.Message, text: string): Promise<Api.Message> => MessageSender.sendOrEdit(current ?? msg, text, { parseMode: 'html' })
...
const getEditErrorText = (error: any): string => {
  const parts = [typeof error?.errorMessage === 'string' ? error.errorMessage : '', typeof error?.message === 'string' ? error.message : ''].filter(Boolean)
  return parts.join(' ')
}

const isMessageNotModifiedError = (error: any): boolean => getEditErrorText(error).includes('MESSAGE_NOT_MODIFIED')
...
class MessageSender {
  ...
  static async sendNew(msg: Api.Message, text: string, options?: MessageOptions, replyToId?: number): Promise<Api.Message> {
    if (!msg.client) {
...
class MessageUtils {
  ...
  async sendLongMessage(msg: Api.Message, text: string, replyToId?: number, token?: AbortToken, options?: { poweredByTag?: string }): Promise<Api.Message> {
    token?.throwIfAborted()
  ...
  async sendImages(msg: Api.Message, images: AIImage[], prompt: string | string[], replyToId?: number, token?: AbortToken, notice?: string): Promise<void> {
  ...
  async sendVideos(msg: Api.Message, videos: AIVideo[], prompt: string, replyToId?: number, token?: AbortToken): Promise<void> {
    const config = (await this.configManagerPromise).getConfig()
      ...
      try {
        ...
        if (pathsToSend.length > 1) {
          ...
          await msg.client.sendFile(peerId, {
            ...
            caption: options.previewEnabled ? caption : pathsToSend.map((_, index) => (index === pathsToSend.length - 1 ? caption : '')),
            parseMode: 'html',
            ...(replyTo ? { replyTo } : {})
          })
  ...
  private async sendHtml(msg: Api.Message, html: string, replyToId?: number, linkPreview?: boolean): Promise<Api.Message> {
...
const isAsyncIterable = (value: any): value is AsyncIterable<any> => !!value && typeof value[Symbol.asyncIterator] === 'function'
...
const collectOpenAISources = (data: any): Array<{ url: string; title?: string }> => {
  ...
  const appendEntries = (entries: any[] | undefined, isAnnotation = false) => {
    ...
    for (const entry of entries) {
      const url = isAnnotation ? entry?.url_citation?.url || entry?.url : entry?.url
      const title = isAnnotation ? entry?.url_citation?.title || entry?.title : entry?.title
  ...
  appendEntries(
    (data?.annotations || []).filter((entry: any) => entry?.type === 'url_citation' || entry?.url_citation),
    true
  )
  appendEntries(
    (choice?.message?.annotations || []).filter((entry: any) => entry?.type === 'url_citation' || entry?.url_citation),
    true
  )
  appendEntries(
    (choice?.delta?.annotations || []).filter((entry: any) => entry?.type === 'url_citation' || entry?.url_citation),
    true
  )
...
const aggregateOpenAIResponses = (
  ...
  for (const payload of payloads) {
    ...
    const fallbackContent = choice?.message?.content ?? choice?.content ?? payload?.content
...
const collectResponsesSources = (item: any): Array<{ url: string; title?: string }> => {
  const sources: Array<{ url: string; title?: string }> = []
  const seen = new Set<string>()
  ...
  const appendActionSources = (action: any) => {
    const sourceList = Array.isArray(action?.sources) ? action.sources : Array.isArray(action) ? action : []
...
const aggregateResponsesApiPayloads = (
  ...
  for (const payload of payloads) {
    ...
    if (payload?.type === 'response.output_text.done' && typeof payload.text === 'string' && deltaTexts.length === 0) {
      fallbackText = payload.text.trim()
    }
    ...
    const response = payload?.response?.object === 'response' ? payload.response : payload?.object === 'response' ? payload : null
...
const parseOpenAIResponseData = async (
  ...
  if (payloads.length > 0) {
    const hasResponsesPayload = payloads.some((payload) => payload?.object === 'response' || payload?.response?.object === 'response' || (typeof payload?.type === 'string' && payload.type.startsWith('response.')))
...
const buildDoubaoVideoUrl = (data: any): string | null => {
  return data?.data?.result?.video_url || data?.data?.output?.video_url || data?.data?.video_url || data?.video_url || data?.content?.video_url || data?.data?.content?.video_url || null
}
...
const extractGeminiVideoResult = (data: any): { uri?: string; bytes?: string } | null => {
  ...
  const sampleUri = response?.generateVideoResponse?.generatedSamples?.[0]?.video?.uri || response?.generate_video_response?.generated_samples?.[0]?.video?.uri
  if (sampleUri) return { uri: sampleUri }

  const videoBytes = response?.generatedVideos?.[0]?.video?.videoBytes || response?.generated_videos?.[0]?.video?.video_bytes || response?.generatedVideos?.[0]?.video?.video_bytes || response?.generated_videos?.[0]?.video?.videoBytes
  if (videoBytes) return { bytes: videoBytes }

  return null
}

const buildGeminiParts = async (prompt: string, images: AIContentPart[], httpClient: HttpClient, token?: AbortToken): Promise<Array<Record<string, any>>> => {
...
class AIService {
  ...
  private applyImageDefaults(request: Record<string, any>, providerConfig: ProviderConfig, model: string, modeConfig: ProviderModeConfig): void {
  ...
  private buildDoubaoVideoContent(prompt: string, images: AIContentPart[], imageMode: VideoImageMode): Array<Record<string, any>> {
  ...
  private async generateVideoWithDoubao(providerConfig: ProviderConfig, model: string, prompt: string, images: AIContentPart[], imageMode: VideoImageMode, videoAudio: boolean, videoDuration: number, modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIVideo[]> {
    ...
    const taskId = response.data?.task_id || response.data?.data?.task_id || response.data?.data?.id || response.data?.id
...
const handleBooleanSetting = async (msg: Api.Message, args: string[], feature: 'image' | 'video', configManager: ConfigManager, config: DB, settings: BooleanSetting[]): Promise<boolean> => {
  ...
  await MessageSender.sendOrEdit(msg, `✅ ${setting.successText}已${enabled ? '开启' : '关闭'}`, { parseMode: 'html' })
  return true
}

const handlePromptOptimizationSetting = async (msg: Api.Message, args: string[], feature: 'image' | 'video', configManager: ConfigManager, config: DB): Promise<boolean> => {
  ...
  if (command.type === 'status') {
    await MessageSender.sendOrEdit(msg, `🧠 <b>${label}提示词优化:</b>\n\n` + `状态: ${enabled ? '开启' : '关闭'}\n` + `长度: <code>${mode}</code>（${labels[mode]}）`, { parseMode: 'html' })
  } else if (command.type === 'toggle') {
    await configManager.updateConfig((cfg) => {
      if (isImage) cfg.promptOptimize = command.enabled
      else cfg.videoPromptOptimize = command.enabled
    })
    await MessageSender.sendOrEdit(msg, `✅ ${label}提示词优化已${command.enabled ? '开启' : '关闭'}`, { parseMode: 'html' })
  } else if (command.type === 'length-status') {
    await MessageSender.sendOrEdit(msg, `📏 ${label}优化提示词长度: <code>${mode}</code>（${labels[mode]}）`, { parseMode: 'html' })
  } else {
    await configManager.updateConfig((cfg) => {
      if (isImage) cfg.promptLength = command.mode
      else cfg.videoPromptLength = command.mode
    })
    await MessageSender.sendOrEdit(msg, `✅ ${label}优化提示词长度已设为 ${command.mode}（${labels[command.mode]}）`, { parseMode: 'html' })
  }
  return true
}
...
class ConfigFeature extends BaseFeatureHandler {
  ...
  private async addConfig(msg: Api.Message, args: string[], configManager: ConfigManager): Promise<void> {
    requireUser(!!(msg as any).savedPeerId, '出于安全考虑，禁止在公开场景添加/修改 API 密钥')
  ...
  private async setProviderBooleanOption(msg: Api.Message, args: string[], configManager: ConfigManager, option: 'stream' | 'responses'): Promise<void> {
  ...
  private async setProviderType(msg: Api.Message, args: string[], configManager: ConfigManager): Promise<void> {
...
class TelegraphFeature extends BaseFeatureHandler {
  ...
  private async deleteTelegraphItem(msg: Api.Message, args: string[], configManager: ConfigManager): Promise<void> {
    ...
    requireUser(!isNaN(idx) && idx >= 0 && idx < config.telegraph.list.length, `序号超出范围 (1-${config.telegraph.list.length})`)
...
class QuestionFeature extends BaseFeatureHandler {
  ...
  cancelCurrentOperation(reason: string = '操作已取消', msg?: Api.Message): boolean {
    if (msg) {
  ...
  private async runQuestion(msg: Api.Message, question: string): Promise<void> {
  ...
  async handleQuestion(msg: Api.Message, question: string, token?: AbortToken): Promise<void> {
  ...
  private async handleLongContentWithTelegraph(msg: Api.Message, question: string, rawAnswer: string, replyToId?: number, token?: AbortToken): Promise<void> {
    const configManager = await this.getConfigManager()
    const config = configManager.getConfig()
    ...
    try {
      ...
      if (referenceCount > 0) {
        ...
        if (messageImageParts.length > 0) {
          ...
          generationStatusMsg = await MessageSender.sendOrEdit(msg, statusText, { parseMode: 'html' })
        }
